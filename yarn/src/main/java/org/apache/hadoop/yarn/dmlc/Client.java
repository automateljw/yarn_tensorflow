...
public class Client {
    ...
    private Client() throws IOException {
        //LOG.info(System.getenv("HADOOP_CONF_DIR") +"/core-site.xml");
        //LOG.info(System.getenv("HADOOP_CONF_DIR") +"/hdfs-site.xml");
        //conf.addResource(new Path(System.getenv("HADOOP_CONF_DIR") +"/core-site.xml"));
        //conf.addResource(new Path(System.getenv("HADOOP_CONF_DIR") +"/hdfs-site.xml"));
        //conf.set("fs.defaultFS", "hdfs://ns3-backup");
    ...
    private Map<String, LocalResource> setupCacheFiles(ApplicationId appId) throws IOException {
        ...
        LOG.info("mkdirs: " + tmpPath);
        FileSystem.mkdirs(dfs, tmpPath, permTemp);
        LOG.info("mkdirs end: " + tmpPath);
        ...
        for (Map.Entry<String, String> e : cacheFiles.entrySet()) {
            ...
            //    path = dst;
            //}
            LOG.info("entry path = " + path);
            if (!e.getValue().startsWith("viewfs://")) {
                Path dst = new Path(tmpPath + "/"+  path.getName());
                LOG.info("dst = " + dst);
                dfs.copyFromLocalFile(false, true, path, dst);
                dfs.setPermission(dst, permTemp);
                //dfs.deleteOnExit(dst);
        ...
        //LOG.info("cacheFile=" + this.cacheFileArg);
        LOG.info("rmap=" + rmap);
        return rmap;
    }
    ...
    private Map<String, String> getEnvironment() {
        ...
        for (Map.Entry<String, String> e : System.getenv().entrySet()) {
            if (e.getKey().startsWith("HADOOP_")) {
                //env.put(e.getKey(), e.getValue());
                env.put(e.getKey(), e.getValue() + "-yarn");
            }
    ...
    private void initArgs(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            if (args[i].equals("-file")) {
                ...
                this.tempdir = args[++i];
            } else if(args[i].equals("-defaultfs")) {
                sargs.append(" ");
                sargs.append(args[i]);
                sargs.append(" ");
                sargs.append(args[i+1]);
                this.defaultFs = args[++i];
    ...
    private void run(String[] args) throws Exception {
        ...
        //System.exit(0);
        LOG.info("CLASSPATH:"+ env.get("CLASSPATH"));
		...
		String cmd = "echo HADOOP_HOME=$HADOOP_HOME classpath=${CLASSPATH} " 
		  		+ " 1>"
		        + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/stdout"
		        + " 2>" + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/stderr";
		String cmd_output_append = " 1>>"
		        + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/stdout"
			    + " 2>>" + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/stderr";

        cmd = cmd + " && " + Environment.JAVA_HOME.$$() + "/bin/java"
          + " -Xmx900m"
          + " org.apache.hadoop.yarn.dmlc.ApplicationMaster"
          + this.cacheFileArg + ' ' + this.appArgs + " 1>>"
          + ApplicationConstants.LOG_DIR_EXPANSION_VAR + "/stdout"
        ...
        //LOG.debug(cmd);
        LOG.info("start AppMaster cmd=" + cmd);
